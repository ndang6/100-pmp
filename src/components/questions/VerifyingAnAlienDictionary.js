...
function VerifyingAnAlienDictionary() {
    const name = "Verifying an Alien Dictionary"
    const hashTags = "#arrays"
    const headerSignature = "public boolean isAlienSorted(String[] words, String order)"

    // \n
    // eslint-disable-next-line no-multi-str
    const prompt = "In an alien language, surprisingly they also use english lowercase letters, but possibly in a different order.\n \
    ...
    // &lt;
    const hint1 = <p>int[] mapping = new int[26];</p>
    const hint2 = <p>if (bigger(words[i - 1], words[i])) return false;</p>
    const hint3 = <p>if (s1.charAt(i) != s2.charAt(i)){"\n"}return mapping[s1.charAt(i) - 'a'] &gt; mapping[s2.charAt(i) - 'a'];</p>
...
export default VerifyingAnAlienDictionary
