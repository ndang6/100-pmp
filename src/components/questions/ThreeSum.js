...
function ThreeSum() {
    const name = "3Sum"
    const hashTags = "#Arrays.sort #searching sorted array"
    const headerSignature = "public List<List<Integer>> threeSum(int[] nums)"

    // \n
    // eslint-disable-next-line no-multi-str
    const prompt = "Given an array nums of n integers, find elements a, b, c in nums such that a + b + c = 0.\n"
    ...
    // &lt;
    const hint1 = <p>if(nums[i] &gt; 0) break;</p>
    const hint2 = <p>if (i == 0 || (i &gt; 0 &amp;&amp; nums[i] != nums[i-1])) ...</p>
    const hint3 = <p>while (lo &lt; hi &amp;&amp; nums[lo] == nums[lo+1]) lo++;{"\n"} while (lo &lt; hi &amp;&amp; nums[hi] == nums[hi-1]) hi--;</p>
