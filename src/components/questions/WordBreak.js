...
function WordBreak() {
    const name = "Word Break"
    const hashTags = "#boolean array #dynamic programming"
    const headerSignature = "public boolean wordBreak(String s, List<String> wordDict)"

    // \n
    // eslint-disable-next-line no-multi-str
    const prompt = "Determine if a string can be segmented into a sequence of one or more dictionary words.\n"
    ...
    // &lt;
    const hint1 = <p>boolean[] f = new boolean[s.length() + 1]</p>
    const hint2 = <p>for(int i = 1; i &lt;= s.length(); i++) ...</p>
    const hint3 = <p> if(f[j] &amp;&amp; wordDict.contains(s.substring(j, i))) ...</p>
