...
function MaximumSubarray() {
    const name = "Maximum Subarray"
    const hashTags = "#dynamic programming"
    const headerSignature = "public int maxSubArray(int[] nums)"

    // \n
    // eslint-disable-next-line no-multi-str
    const prompt = "Given an integer array nums, find the contiguous subarray which has the largest sum, and return its sum."
    ...
    // &lt;
    const hint1 = <p>int[] dp = new int[nums.length]</p>
    const hint2 = <p>dp[i] =  nums[i] + ...</p>
    const hint3 = <p>max = Math.max(max, dp[i])</p>
