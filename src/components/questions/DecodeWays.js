...
function DecodeWays() {
    const name = "Decode Ways"
    const hashTags = "#dynamic programming"
    const headerSignature = "public int numDecodings(String s)"

    // \n
    // eslint-disable-next-line no-multi-str
    const prompt = "A message containing letters from A-Z is being encoded to numbers.\n \
                    'A' -> 1, 'B' -> 2, ... , 'Z' -> 26\n \
    ...
    // &lt;
    const hint1 = <p>int[] dp = new int[s.length() + 1]</p>
    const hint2 = <p>dp[0] = ?{"\n"}dp[1] = ?</p>
    const hint3 = <p>substring(i - 1, i), substring(i - 2, i)</p>
